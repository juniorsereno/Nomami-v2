...
export async function POST(request: Request) {
  ...
  try {
    ...
    const paymentValue = body.payment?.value;
    const nextDueDate = new Date();
    nextDueDate.setDate(nextDueDate.getDate() + 30);

    await sql`
      INSERT INTO subscribers (name, email, cpf, phone, plan_type, start_date, next_due_date, status, asaas_customer_id, value)
      VALUES (${name}, ${email}, ${cpfCnpj}, ${mobilePhone}, 'mensal', NOW(), ${nextDueDate.toISOString()}, 'active', ${customerId}, ${paymentValue})
