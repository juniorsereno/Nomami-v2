...
export async function POST(request: Request) {
  try {
    ...
    const { company_name, cnpj, phone, address, category, benefit_description, status, logo_url } = validation.data;

    const result = await sql`
      INSERT INTO parceiros (nome, cnpj, categoria, beneficio, ativo, updated_at, endereco, telefone, logo_url)
      VALUES (${company_name}, ${cnpj}, ${category}, ${benefit_description}, ${status === 'ativo'}, NOW(), ${address}, ${phone}, ${logo_url || null})
      RETURNING id;
    `;
